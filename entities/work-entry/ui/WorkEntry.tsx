...
export function WorkEntry({ entry }: WorkEntryProps) {
  return (
    <div className="relative group pb-6 md:pb-0">
      <div className="absolute -inset-2 bg-gradient-to-r from-[#00ffcc]/5 to-transparent rounded-sm blur opacity-0 group-hover:opacity-100 transition duration-300"></div>
      <div className="relative">
        <div className="mb-3">
          <h3 className="text-[#00ffcc] mb-1">{entry.title}</h3>
        ...
        <ul className="space-y-2 text-gray-400 text-sm">
          {entry.achievements.map((achievement, idx) => (
            <li key={idx} className="flex gap-3">
              <span className="text-[#00ffcc]/60 flex-shrink-0">▹</span>
